#load ".paket/load/net6.0/Suave.fsx"
...
let sock = 
  { defaultConfig.bindings.[0].socketBinding with port = 1414us }
let config =
  { defaultConfig with
      homeFolder = Some (Path.GetFullPath "./output")
      bindings = [ { defaultConfig.bindings.[0] with socketBinding = sock } ]
      compressedFilesFolder = Some(Path.GetTempPath()) }

let _, start = startWebServerAsync config app
//Async.Start(start)
Process.Start(ProcessStartInfo("http://localhost:1414/", UseShellExecute=true))
