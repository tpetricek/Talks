...
let chartStyle ch =
  let grid = ChartTypes.Grid(LineColor=System.Drawing.Color.DimGray)
  ch 
...
let wrapFsiEvaluator (fsiEvaluator:FsiEvaluator) root output (floatFormat:string) =
  ...
  let transformation (value:obj, typ:System.Type) =
    ...
    | :? ChartTypes.GenericChart as ch ->
        ...
        ( let ch = chartStyle ch
          use chart = new ChartControl(ch, Dock = DockStyle.Fill, Width=650, Height=400)
          ...
          do chart.ForeColor <- Color.White
