...
let fsiEvaluator =
    ...
    evaluator.RegisterTransformation(fun (v, typ) ->
      if v = null then None else
      let mi = v.GetType().GetMethod("AsLatex")
      if mi = null then None else
      let latex = mi.Invoke(v, [||]) :?> string
      Some [FSharp.Markdown.MarkdownParagraph.LatexBlock [latex]] )
    evaluator
...
Target "GenerateSlides" (fun _ ->
    // Overwrite the default template with my custom one!
