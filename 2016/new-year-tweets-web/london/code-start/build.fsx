...
// The loaded WebPart is then hosted at localhost:8081.
// --------------------------------------------------------------------------------------
...
let reloadScript () =
  try
    ...
    try
      fsiSession.EvalInteraction("cleanup()")
      printfn "Cleaned up last session."
    with e -> printfn "Cleanup not configured."
...
let serverConfig =
  { defaultConfig with
      ...
      bindings = [ HttpBinding.mkSimple HTTP  "127.0.0.1" 8081] }
